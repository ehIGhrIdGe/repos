...
namespace QuizSample
{
    static class ManagerDb
    {
        ...
        /// <param name="tableType">取得したいテーブルのタイプ</param>
        /// /// <param name="dataTable">値をセットするデータテーブル</param>
        public static bool TryGetWhere(TABLETYPE tableType, WHERETYPE whereType, int id, out DataTable dataTable)
        {
            ...
            if(id > 0)
            {
                switch (whereType)
                {
                    case WHERETYPE.QuizId:
                        condition = $" WHERE quiz_id = {id}";
                        break;
                    case WHERETYPE.CategoryId:
                        condition = $" WHERE category_id = {id}";
                        break;
                    case WHERETYPE.ChoicesId:
                        condition = $" WHERE choices_id = {id}";
                        break;
                }
            }
            ...
            try
            {
                ...
                using (SqlDataAdapter ada = new SqlDataAdapter())
                {
                    con.Open();
                    cmd.Connection = con;
                    cmd.CommandText = ConfigurationManager.AppSettings[tableType.ToString()] + condition;
